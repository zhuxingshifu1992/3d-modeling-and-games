...
import math
import random
from interiors import make_building, furnish_space, add_landmark_access
...
def build(B):
    ...
    for i in range(len(path)-1):
        for j in range(cross_sections):
            ...
            # Cross-sections advance from right to left; this winding keeps
            # the road's top face upward for single-sided game collision.
            road_faces.extend([(a,b+1,a+1),(a,b,b+1)])
    ...
    def facade(origin,width,base,storeys,bays,floor_height,rot,entrance=False):
        bay=width/bays
        for row in range(storeys):
            ...
            for col in range(bays):
                ...
                if entrance and row==0 and col==bays//2:
                    wall_cell(origin,u,bottom,top,bay,2.2,bottom,3.45,rot,glazed=False)
                else:
    ...
    def palace(cx,cy,w,d,base,storeys,bays_w,bays_d,fh,roof_h,bid,label,entry_side):
        eave=base+storeys*fh
        facade((cx,cy-d/2),w,base,storeys,bays_w,fh,0)
        facade((cx,cy+d/2),w,base,storeys,bays_w,fh,math.pi)
        facade((cx+w/2,cy),d,base,storeys,bays_d,fh,math.pi/2,entry_side==1)
        facade((cx-w/2,cy),d,base,storeys,bays_d,fh,-math.pi/2,entry_side==-1)
        B.box('DE_Foundations',(cx,cy,base-.04),(w+.6,d+.6,.16),'stone')
        # Put the stairs in the southeast interior corner: the northwest
        # corner is occupied by the large attached circular tower.
        record=make_building(B,bid,label,cx,cy,d,w,base,storeys*fh,'castle',floors=storeys,front_angle=-math.pi/2,shell=False)
        record.update({'entrance':[cx+entry_side*(w/2+1.5),cy,base+.08],
                       'inside':[cx+entry_side*(w/2-1.5),cy,base+.13],
                       'front_angle':entry_side*math.pi/2})
    ...
    palace(-15,14,19,35,12.23,5,5,9,5.6,10.1,'DE_PALAS','王宫主楼',1)
    palace(19.7,7.0,9.5,29,12.23,3,3,7,6.5,7.2,'DE_KNIGHTS','骑士宫',-1)

    def ring_wall(name,cx,cy,outer,inner,z0,z1,mat,n=24):
        verts=[]
        for zz in (z0,z1):
            for rr in (outer,inner):
                verts.extend((cx+rr*math.cos(i*tau/n),cy+rr*math.sin(i*tau/n),zz) for i in range(n))
        faces=[]
        for i in range(n):
            j=(i+1)%n
    ...
    def tower(cx,cy,r,base,top,spire,slender=False,bid=None,label=None,source=None):
        n=24
        if bid:
            ring_wall('DE_Tower_Masonry',cx,cy,r,r-.36,base,top,'white',n)
            floor=top-4.0
            B.cyl('IN_'+bid+'_Landing',(cx,cy,floor-.08),r-.36,.16,'marble',n)
            B.cyl('COL_'+bid+'_Landing',(cx,cy,floor-.08),r-.36,.16,'stone',n)
            size=(r-.46)*1.40
            rec=make_building(B,bid,label,cx,cy,size,size,floor,3.65,'castle',floors=1,shell=False)
            target=[cx,cy-(.48 if r<2.3 else 0),floor+.12]
            rec.update({'entrance':source,'inside':target,'access':'lift'})
            add_landmark_access(B,bid+'_UP','登上'+label,source,target,'lift')
            add_landmark_access(B,bid+'_DOWN','返回城堡内院',target,source,'lift')
            B.box('DE_Lift_Markers',(source[0]+.75,source[1],source[2]+.52),(.30,.30,1.04),'bronze')
            B.box('DE_Lift_Markers',(source[0]+.75,source[1]-.17,source[2]+.88),(.23,.035,.22),'emissive')
        else:
        ...
        for z in (base+3.6,base+9.2,base+14.8,base+20.4,top-1.1,top-.25):
            if z<top:
                if bid:
                    ring_wall('DE_Tower_Cornices',cx,cy,r+.13,r-.36,z-.09,z+.09,'limestone',n)
                else:
    ...
    tower(-23.3,29,4.1,12.23,49.4,12.5,bid='DE_TOWER_NW',label='西北圆塔观景室',source=[2,20,12.33])
    tower(-24.2,-2.9,3.3,12.23,48.5,10.2,bid='DE_TOWER_SW',label='西南圆塔观景室',source=[2,4,12.33])
    tower(-6.0,29.0,3.1,12.23,49.0,10.4,bid='DE_TOWER_NE',label='王宫北圆塔观景室',source=[6,22.5,12.33])
    tower(23.6,21.5,3.35,12.23,46.0,11.7,bid='DE_TOWER_E',label='骑士宫圆塔观景室',source=[11,19,12.33])
    tower(15.1,-6.8,2.0,12.23,39.8,8.3,True,bid='DE_TOWER_S',label='内院尖塔观景室',source=[10,-5,12.33])

    # Squared keep beside the Palas with smaller corner turrets.
    kx,ky=-14.4,-7.8
    make_building(B,'DE_KEEP','王宫方塔',kx,ky,9.4,8.4,12.23,35.15,'castle',floors=6)
    for z in (17,22.6,28.2,33.8,39.4,46.7):
        for sign in (-1,1):
            B.box('DE_Tower_Cornices',(kx+sign*4.67,ky,z),(.36,8.7,.23),'limestone')
            B.box('DE_Tower_Cornices',(kx,ky+sign*4.17,z),(9.7,.36,.23),'limestone')
    for side in (-1,1):
    ...
    gate_origin=(8,-24.2)
    wall_cell(gate_origin,0,12.22,24.5,8.6,4.8,12.22,6.3,0,'brick',.55,False)
    wall_cell((8,-18.5),0,12.22,24.5,8.6,4.8,12.22,6.3,math.pi,'brick',.55,False)
    for xx in (3.72,12.28):
        B.box('DE_Gatehouse',(xx,-21.35,18.36),(.42,5.7,12.28),'brick')
    B.arch('DE_Gatehouse',(8,-18.47,12.22),4.8,6.3,.30,'limestone',0,12,.45)
    for cx,w,bid,label in ((-.75,8.9,'DE_GATE_W','西门楼卫兵馆'),(17.25,9.9,'DE_GATE_E','东门楼历史馆')):
        # Hollow red masonry sides, with the door opening onto the inner court.
        B.box('DE_Gatehouse',(cx,-23.95,18.36),(w,.50,12.28),'brick')
        for sign in (-1,1):
            B.box('DE_Gatehouse',(cx+sign*(w/2-.25),-21.35,18.36),(.50,5.7,12.28),'brick')
        wall_cell((cx,-18.5),0,12.22,24.5,w,2.2,12.22,3.35,math.pi,'brick',.50,False)
        wing=make_building(B,bid,label,cx,-21.35,w,5.7,12.22,12.28,'castle',floors=3,front_angle=math.pi,shell=False)
        # The shared kit provides compact-room lifts and complete floor slabs.
        wing['vertical_access']='lift'
        for z in (15.6,20.6,24.55):
            for sign in (-1,1):
                B.box('DE_Gatehouse',(cx+sign*(w/2-.05),-21.35,z),(.28,5.9,.24),'limestone')
                B.box('DE_Gatehouse',(cx,-21.35+sign*2.80,z),(w+.18,.30,.24),'limestone')
    ...
    loft=make_building(B,'DE_GATE_LOFT','门楼徽章陈列厅',8,-21.35,7.5,4.65,19.5,4.75,'castle',floors=1,shell=False)
    loft_source=[8,-15.8,12.33]; loft_target=[8,-21.35,19.62]
    loft.update({'entrance':loft_source,'inside':loft_target,'access':'lift'})
    add_landmark_access(B,'DE_GATE_LOFT_UP','进入门楼陈列厅',loft_source,loft_target,'lift')
    add_landmark_access(B,'DE_GATE_LOFT_DOWN','返回内院',loft_target,loft_source,'lift')
    ...
    return {'landmark':'新天鹅堡风格山顶城堡',
            ...
            'spawn':[-17,-55,.30],
            'description':'可进入的多层王宫、骑士宫、方塔与门楼，五间圆塔观景室可由内院升降访问；保留石砌拱窗、板岩陡屋顶、开放庭院与山林道路。',
            'reference':'https://www.neuschwanstein.de/englisch/palace/',
