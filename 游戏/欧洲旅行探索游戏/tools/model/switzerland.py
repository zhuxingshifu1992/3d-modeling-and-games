...
def build(B):
    ...
    def chalet(x, y, w, d, h, variant):
        ...
        bid='CH_CHALET_%02d' % (variant+1)
        make_building(B,bid,'阿尔卑斯木屋 %02d' % (variant+1),x,y,w,d,.02,h-.02,'swiss',floors=2)
        ...
        for zz in (.98, 2.92, h - .13):
            if zz<2.85:
                half=(w+.32-2.3)/2
                for sign in (-1,1):
                    B.box('SW_TimberStructure',(x+sign*(1.15+half/2),front-.16,zz),(half,.23,.21),'dark_wood')
            else:
        ...
        # Door leaves are parked against the inner jambs, leaving the center
        # completely open. Ground floor is flush with the village approach.
        for sign in (-1,1):
            B.box('SW_Doors',(x+sign*1.17,front+.70,1.41),(.10,1.22,2.66),'dark_wood')
            B.box('SW_Doors',(x+sign*1.08,front+.95,1.43),(.045,.25,.085),'bronze')
            B.box('SW_Foundations',(x+sign*1.15,front-.14,1.43),(.22,.38,2.86),'stone')
        B.box('SW_Foundations',(x,front-.14,2.99),(2.52,.38,.25),'stone')
        B.box('SW_Foundations',(x,front-.38,.075),(2.18,.82,.11),'stone')
        for sx in (-1, 1):
    ...
    x, y = -51, 1
    church=make_building(B,'CH_CHURCH','山村教堂',x,y,7.8,13,.02,7.42,'church',floors=1)
    B.roof('SW_Roofs', (x, y, 7.44), 8.8, 14, 4.2, 'slate')
    tx, ty = x, y - 7.2
    # Open front AND back of the belltower form a vestibule into the nave.
    for sign in (-1,1):
        B.box('SW_Church',(tx+sign*2.10,ty,7.35),(.40,4.6,14.7),'plaster')
        for yy in (ty-2.1,ty+2.1):
            B.box('SW_Church',(tx+sign*1.65,yy,1.60),(1.10,.40,3.2),'plaster')
    for yy in (ty-2.1,ty+2.1):
        B.box('SW_Church',(tx,yy,8.95),(4.6,.40,11.5),'plaster')
    B.box('SW_Foundations',(tx,ty,.065),(4.15,4.60,.10),'paving')
    for zz in (4.9,9.9,14.4):
    ...
    B.arch('SW_ChurchDetails',(tx,ty-2.37,.05),2.2,3.3,.27,'stone',thickness=.22)
    for sign in (-1,1):
        B.box('SW_Doors',(tx+sign*1.24,ty-1.25,1.43),(.11,1.8,2.7),'dark_wood')
    church['entrance']=[tx,ty-3.2,.12]
    church['inside']=[tx,y-3.6,.12]
    # A furnished clock room can be visited from the nave via the bell lift.
    clockroom=make_building(B,'CH_CHURCH_TOWER','教堂钟楼观景室',tx,ty,3.5,3.5,10.03,4.15,'church',floors=1,shell=False)
    source=[tx,y+2,.12]; target=[tx,ty,10.19]
    add_landmark_access(B,'CH_BELL_LIFT','登上教堂钟楼',source,target,'lift')
    add_landmark_access(B,'CH_BELL_RETURN','返回教堂中殿',target,source,'lift')
    clockroom.update({'entrance':source,'inside':target,'access':'lift'})
    ...
    return {
        ...
        'spawn': [0, -54, 1.8],
        'description': '不对称岩雪山峰、八栋可进入的双层阿尔卑斯木屋、带钟楼观景室的开放教堂、溪流木桥与村道；雪线和地形非测绘数据。',
