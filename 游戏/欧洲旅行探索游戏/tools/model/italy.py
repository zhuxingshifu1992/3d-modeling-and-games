...
def build(B):
    ...
    street_labels=('罗马历史书店','罗马咖啡馆','古典雕塑画廊','意式手工艺坊',
                   '旅行者旅馆','罗马小餐馆','建筑档案馆','特拉维丁石材展厅',
                   '意大利美术馆','罗马阅读室','地中海旅馆','意式烘焙屋',
                   '古币陈列馆','罗马旅客会馆')
    street_buildings=[]
    ...
    def building(x, y, w, d, floors, facing, index):
        ...
        # The kit's local front is -Y. Side streets rotate the local footprint.
        rot = facing + math.pi / 2
        side_facing = abs(math.sin(facing)) < .5
        shell_w, shell_d = (d, w) if side_facing else (w, d)
        base = .15
        record = make_building(B, f'it_street_{index+1:02d}', street_labels[index],
                               x, y, shell_w, shell_d, base, floors*3.10,
                               'italian', floors=floors, front_angle=rot, shell=True)
        street_buildings.append(record)
        def local_box(name,u,v,z,size,material):
            B.box(name,(x+u*math.cos(rot)-v*math.sin(rot),
                        y+u*math.sin(rot)+v*math.cos(rot),z),size,material,rot)
        def perimeter(name,z,thickness,overhang,material,door_gap=False):
            depth=.22
            local_box(name,0,shell_d/2+overhang/2,z,
                      (shell_w+overhang*2,depth+overhang,thickness),material)
            for sign in (-1,1):
                local_box(name,sign*(shell_w/2+overhang/2),0,z,
                          (depth+overhang,shell_d,thickness),material)
            if door_gap:
                wing=(shell_w+overhang*2-2.2)/2
                for sign in (-1,1):
                    local_box(name,sign*(1.1+wing/2),-shell_d/2-overhang/2,z,
                              (wing,depth+overhang,thickness),material)
            else:
                local_box(name,0,-shell_d/2-overhang/2,z,
                          (shell_w+overhang*2,depth+overhang,thickness),material)
        perimeter('Street_Trim',.57,.84,.065,'stone',True)
        for z in (3.30, height - .17, height + .12):
            perimeter('Street_Trim',z,.19,.125,'limestone')
        # Shallow attic perimeter closes the gap to the retained terracotta roof.
        perimeter('Street_Facades',base+floors*3.10+.30,.60,0,mat)
        ...
        for level in range(floors):
            for col in range(cols):
                ...
                for sign in (-1, 1):
                    if level == 0 and abs(u + sign * .88) < 1.4:
                        continue
                ...
                if level == 1 and col % 2 == 0:
                    ...
                    for rail in range(7):
                        item('Street_Balconies', u - .72 + rail * .24, z - .51,
                             (.035, .035, .65), 'metal', .97)
        # Open jambs replace the old opaque wooden door slab.
        for u in (-1.17, 1.17):
            item('Street_Trim', u, 1.625, (.22, .36, 2.95), 'limestone', .19)
            item('Street_Trim', u, 3.06, (.34, .44, .16), 'limestone', .21)
        item('Street_Trim', 0, 3.23, (2.64, .41, .22), 'limestone', .19)
        item('Street_Doors', 1.64, 1.67, (.42, .07, .54), 'bronze', .24)
        for sign in (-1, 1):
    ...
    # Small archaeological exhibits line the ambulatory without obstructing
    # the three cardinal access routes or using the open arena as a room.
    for index,t in enumerate((.72,2.42,3.88,5.56)):
        x,y,_=ellipse(34.85,27.85,t)
        B.cyl('Colosseum_Exhibits',(x,y,.58),.36,.82,'limestone',12)
        B.cyl('Colosseum_Exhibits',(x,y,1.34),.17,.72,'marble',12,radius_top=.14)
        B.box('Colosseum_Exhibits',(x,y,1.72),(.43,.36,.12),'limestone',t)
        # Carved capital and a separately mounted bronze interpretation label.
        for sign in (-1,1):
            B.cyl('Colosseum_Exhibits',(x+sign*.13,y,1.81),.095,.13,'marble',8)
        nx,ny=math.cos(t),math.sin(t)
        B.box('Colosseum_Exhibits',(x+nx*.31,y+ny*.31,.88),(.28,.035,.22),'bronze',t+math.pi/2)
    ...
    return {
        ...
        'street_building_count':len(street_buildings),
        'street_building_ids':[f'it_street_{i+1:02d}' for i in range(14)],
        'street_building_labels':list(street_labels),
        'interior_style':'italian',
    }
