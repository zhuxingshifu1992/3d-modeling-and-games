...
ASSET=ROOT/'assets/models/unicorn_refined.glb'
REPORT=ROOT/'previews/refined_unicorn/full_body_hatch_sweep.json'
manifest=json.loads((ROOT/'previews/refined_unicorn_game/export_manifest.json').read_text(encoding='utf-8'))
...
print('UNICORN_FULL_BODY_HATCH_SWEEP',report['status'],'initial',report['initial_closed_contacts'],'moving_poses',len(moving_contacts),'after_slide',len(after),'final',report['final_open_contacts'],flush=True)
