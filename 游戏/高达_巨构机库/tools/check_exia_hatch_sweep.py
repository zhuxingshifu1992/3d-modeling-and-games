...
ASSET=ROOT/'assets/models/exia_refined.glb'
REPORT=ROOT/'previews/refined_exia/full_body_hatch_sweep.json'
manifest=json.loads((ROOT/'previews/refined_exia_game/export_manifest.json').read_text(encoding='utf-8'))
...
report={'asset_sha256':hashlib.sha256(ASSET.read_bytes()).hexdigest(),'profile':profile,
        ...
        'initial_closed_contacts':rows[0]['overlap_pairs'],'after_slide_contact_poses':len(after),
        'final_open_contacts':len(open_pairs),'samples':rows,'examples':examples,
        'status':'passed' if not after and not open_pairs else 'failed'}
REPORT.write_text(json.dumps(report,indent=2),encoding='utf-8')
print('EXIA_FULL_BODY_HATCH_SWEEP',report['status'],'initial',report['initial_closed_contacts'],'after_slide',len(after),'final',report['final_open_contacts'],flush=True)
if after or open_pairs:raise RuntimeError('Hatch intersects game geometry after initial slide; inspect report.')
